...
export const AccountSettings = ({ user, onComplete, onBack }) => {
    ...
    const loadData = React.useCallback(async () => {
        ...
        setFormData(prev => ({
            ...prev,
            repName: data.repName || data.name || user.displayName || '',
    ...
    const handleCapture = async () => {
        ...
        canvas.toBlob(async (blob) => {
            ...
            const res = await uploadAccountPhoto(user.uid, file);
            if (res && res.status === 'success') setFormData(p => ({ ...p, photoURL: res.url }));
        }, 'image/jpeg', 0.85);
    };
    ...
    const handleChangePassword = async () => {
        if (!pwData.newPw || pwData.newPw.length < 6) {
            setPwMessage({ text: 'A nova senha deve ter pelo menos 6 caracteres.', type: 'error' });
            return;
        }
        if (pwData.newPw !== pwData.confirm) {
            setPwMessage({ text: 'A nova senha e a confirmação não coincidem.', type: 'error' });
            return;
        }
        setPwLoading(true);
        ...
        setPwLoading(false);
        if (res.status === 'success') {
            setPwMessage({ text: '✅ Senha atualizada com sucesso!', type: 'success' });
            setPwData({ current: '', newPw: '', confirm: '' });
        } else {
            setPwMessage({ text: res.message, type: 'error' });
        }
    };

    // Check if profile needs completion (first time)
    const isProfileIncomplete = !formData.repName || !formData.repPhone;
                ...
                <div className="mt-3">
                    ...
                    <div className="flex gap-4 items-end mb-5">
                        ...
                        <div className="flex flex-col justify-end gap-1.5">
                            ...
                            <p className="text-xl font-bold text-slate-800 leading-tight">{formData.repName || '—'}</p>
                ...
                <div className="mt-4">
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) => setFormData(p => ({ ...p, repName: e.target.value }))}
                        ...
                        <div className="my-6 text-center p-4 bg-slate-50 rounded-lg border border-slate-200 dashed">
                            <label className="block text-sm font-semibold text-slate-700 mb-2">Foto Atual</label>
                            {cameraOpen ? (
                                ...
                                <div className="flex flex-col items-center gap-3">
                                    ...
                                    <div className="flex flex-wrap items-center justify-center gap-2">
                                        ...
                                        >
                                            <Camera size={18} />
                                            {formData.photoURL ? 'Atualizar Foto' : 'Tirar Foto Agora'}
                                        </button>
                                        <label className="flex items-center gap-2 bg-slate-200 text-slate-700 px-4 py-2 rounded-lg hover:bg-slate-300 transition cursor-pointer text-sm font-medium">
                                            <span>📂 Galeria</span>
                                            <input
                                                type="file"
                                                accept="image/*"
                    ...
                    <div className="mt-6 border-t border-slate-200 pt-5">
                        ...
                        <div className="space-y-2">
                            ...
                            />
                            {pwMessage.text && (
                                <p className={`text-xs font-medium px-1 ${ pwMessage.type === 'success' ? 'text-green-600' : 'text-red-600' }`}>
                                    {pwMessage.text}
                                </p>
                            )}
                            <button
                                ...
                                disabled={pwLoading || !pwData.current || !pwData.newPw}
                                className="w-full py-2 bg-slate-700 text-white text-sm font-semibold rounded-lg hover:bg-slate-800 transition disabled:opacity-40 active:scale-95"
                            >
