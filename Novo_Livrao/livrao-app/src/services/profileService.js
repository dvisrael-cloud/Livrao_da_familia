        ...
        try {
            ...
            if (file.size < 2 * 1024 * 1024) {
                ...
                const timeoutPromise = new Promise((_, reject) =>
                    setTimeout(() => {
                        uploadTask.cancel();
                        reject(new Error("Timeout: O envio do arquivo grande demorou muito (60s)."));
                    }, 60000)
                );
...
/**
 * Busca dados da CONTA do usuário (Coleção 'familias')
 * @param {string} uid
 */
...
/**
 * Atualiza dados da CONTA do usuário (Coleção 'familias')
 * @param {string} uid
...
export const updateAccountData = async (uid, data) => {
    ...
    try {
        ...
        // Mapear 'name' para 'repName' e manter compatibilidade com Form/Auth
        const updateData = { ...data };
        if (updateData.name) {
            updateData.repName = updateData.name;
        }
 ...
 * Salva em familias/{uid}/profile_pic.jpg
 */
export const uploadAccountPhoto = async (uid, file) => {
    if (!uid || !file) return { status: 'error', message: 'Arquivo inválido' };
    try {
        // Caminho: familias/UID/profile_pic.jpg
        const storagePath = `familias/${uid}/profile_pic.jpg`;
        const storageRef = ref(storage, storagePath);
