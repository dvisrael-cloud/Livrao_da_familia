...
function App() {
  ...
  const progressComponent = view === 'form' ? (
    <div className="w-full flex justify-start">
      <div className="flex flex-col gap-0.5 pb-0.5 w-fit max-w-full">
        {/* Linha 2: LIVRÃO DA FAMÍLIA */}
        <span className="text-[14px] sm:text-3xl font-bold text-green-900 uppercase tracking-widest leading-tight whitespace-nowrap font-playfair">
          LIVRÃO DA FAMÍLIA
        </span>
        {/* Linha 3: Barra + XX% */}
        <div className="flex items-center gap-2 w-full">
          <div className="flex-1 h-1 bg-black/10 rounded-full overflow-hidden">
            <div
              className="h-full shadow-[0_0_12px_rgba(37,99,235,0.4)] transition-all duration-700 ease-out bg-blue-600"
              style={{ width: `${currentPercentage}%` }}
            />
          </div>
          <span className="text-[10px] font-bold text-slate-700 uppercase tracking-wider">{currentPercentage}%</span>
        </div>
        {isEditingMember && progressInfo?.roleLabel && (
          <div className="text-[11px] text-slate-800 font-medium truncate animate-fade-in">
    ...
    <MainLayout
      ...
      headerTitle={
        view === 'account' ? (
          ...
          <span className="text-xl sm:text-4xl font-bold text-green-900 uppercase tracking-widest leading-tight whitespace-nowrap font-playfair">
            Livrão da Família
          </span>
        ) : (
          <span className="text-[11px] sm:text-[13px] tracking-[0.2em] text-slate-900 font-bold whitespace-nowrap">
            Árvore Familiar
          </span>
        )
      }
      ...
      navBar={
        (view === 'form' && isEditingMember && progressInfo?.sectionName !== 'General') ? (
          <FormNavBar
          ...
          <div className="flex items-center gap-2">
            ...
            {view === 'form' && formStep !== 2 ? (
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-slate-500 hover:text-red-600 hover:border-red-200 transition-all font-bold text-[10px] uppercase tracking-wider shadow-sm group active:scale-95"
              >
    ...
    >
      ...
      {userSession && view === 'form' && (
        <>
          ...
          />
          {!uploading && (
            <div className="text-center mt-2 text-xs text-stone-300">versão para teste 26/02/26</div>
          )}
        </>
      )}
