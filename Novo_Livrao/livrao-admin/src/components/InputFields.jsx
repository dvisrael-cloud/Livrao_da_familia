...
export const InputField = ({ label, type = 'text', value, onChange, placeholder, required = false, className = '', inputClassName = '' }) => (
    <div className={`flex flex-col gap-1 ${className}`}>
        {label && <label className="text-sm font-semibold text-history-green">{label} {required && '*'}</label>}
        <input
            type={type}
            value={value || ''}
            onChange={onChange}
            placeholder={placeholder}
            className={`p-2 border border-stone-300 rounded-md bg-stone-50 focus:ring-2 focus:ring-gold-accent focus:border-transparent outline-none transition-all shadow-sm ${inputClassName}`}
        />
    </div>
);
