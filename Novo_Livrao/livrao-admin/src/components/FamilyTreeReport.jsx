...
export const FamilyTreeReport = ({ family, members, isLoading, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-stone-900/90 z-50 overflow-y-auto flex items-center justify-center p-4">
            <div className="bg-[#Fdfbf7] w-full max-w-[1400px] min-h-[90vh] rounded-xl shadow-2xl relative flex flex-col items-center py-10 px-4 overflow-hidden print:w-full print:max-w-none print:h-auto print:bg-white">
                ...
                {/* Title */}
                <h1 className="text-5xl font-serif font-black text-black mb-16 z-10 uppercase tracking-widest text-center border-b-2 border-stone-300 pb-4 px-10 relative">
                    Árvore de Família
                    {isLoading && (
                        <div className="absolute -top-12 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-white/80 px-4 py-1.5 rounded-full shadow-sm text-sm text-stone-600 font-sans tracking-normal normal-case animate-pulse">
                            <div className="w-3.5 h-3.5 border-2 border-stone-600 border-t-transparent rounded-full animate-spin"></div>
                            Buscando membros...
                        </div>
                    )}
                </h1>
